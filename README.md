...
2. Mounts user's podman storage to `/var/tmp/storage` inside the container (simple path that exists in container)
3. Passes `--root /var/tmp/storage` to `bootc-base-imagectl` to access the mounted storage correctly
